...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement) {
        this.container = container;
        // Prefer global listeners so the visual layer can be `pointer-events: none` and not block UI.
        // Attach to window/document and fall back to container-based listeners if needed.
        try {
          window.addEventListener('mousemove', this._onMouseMove);
          window.addEventListener('touchstart', this._onTouchStart, { passive: true } as AddEventListenerOptions);
          window.addEventListener('touchmove', this._onTouchMove, { passive: true } as AddEventListenerOptions);
          window.addEventListener('touchend', this._onTouchEnd as EventListener);
      ...
      dispose() {
        ...
        if (!c) return;
        c.removeEventListener('pointerenter', this._onMouseEnter);
        c.removeEventListener('pointerleave', this._onMouseLeave);
        // Also remove possible fallback listeners
